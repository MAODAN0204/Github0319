...
namespace wyer.Views
{
    ...
    public partial class LoginPage : ContentPage
    {
        ...
        private async void Button_Clicked(object sender, EventArgs e)
        {
            if ((username.Text != null) && (password.Text != null))
            {
                ...
                }catch(Exception ex)
                {
                    ...
                    await DisplayAlert("Error", "Contact System Administrator", "OK");
                }
                finally
                {
                    client.Dispose();
        ...
        {
            var weburl = new Views.ControlPage("http://118.163.50.93/register-alpsun");
            await Navigation.PushAsync(weburl);
        }

        private async void TapGestureForgetPS_Tapped(object sender, EventArgs e)
        {
            var weburl = new Views.ControlPage("http://118.163.50.93/forget-alpsun");
            await Navigation.PushAsync(weburl);
        }
        ...
        private async Task<String> InputUserInfo()
        {
            ...
            // WebRequest request = WebRequest.Create("http://192.168.1.146/member");
            // request.Method = "POST";
            var client = new HttpClient();
            String resultString = "";
            try
