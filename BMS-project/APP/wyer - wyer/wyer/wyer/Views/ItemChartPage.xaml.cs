                ...
                }catch(Exception ex)
                {
                    if (ex.Message.IndexOf("Failed to connect") != -1)
                    {
                        ...
                        {   //網路斷線
                            ...
                            if (pingResult == false && HomePage.connected == true)
                            {
                ...
                switch (str)
                {
                    ...
                    case "temp1":
                        if (float.Parse(json[i].A005) == 0)
                            ...
                            temp = float.Parse(json[i].A005) - 273;
                        orderCounts.Add(new DeviceOrdersCount { Titalname = shorttime, OrderCount = float.Parse(Math.Round(temp*float.Parse("0.01"),2).ToString()) });
                        break;
                    case "temp2":
                        if (float.Parse(json[i].A006) == 0)
                            temp = 0;
                        else
                            temp = float.Parse(json[i].A006) - 273;
                        orderCounts.Add(new DeviceOrdersCount { Titalname = shorttime, OrderCount = float.Parse(Math.Round(temp * float.Parse("0.01"),2).ToString()) });
                        break;
                    case "temp3":
                        if (float.Parse(json[i].A007) == 0)
                            ...
                            temp = float.Parse(json[i].A007) - 273;
                        orderCounts.Add(new DeviceOrdersCount { Titalname = shorttime, OrderCount = float.Parse(Math.Round(temp * float.Parse("0.01"),2).ToString()) });
                        break;
                    case "temp4":
                        ...
                        orderCounts.Add(new DeviceOrdersCount { Titalname = shorttime, OrderCount = float.Parse(Math.Round(float.Parse(json[i].A008) * float.Parse("0.01"),2 ).ToString())});
                        break;
                    case "temp5":
        ...
        private List<RealList> Getlabel(List<SinglefieldArray> json, String str)
        {
            ...
            for (i = 0; i < json.Count(); i++)
            {
                ...
                switch (str)
                {
                    case "power2":
                        float totalvalue2 = 0;
                        totalvalue2 = float.Parse((Math.Round(float.Parse(json[i].A002) * float.Parse(json[i].A004) * float.Parse("0.000001") * float.Parse("0.00111"), 2)).ToString());
